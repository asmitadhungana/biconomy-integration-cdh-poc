...
let config = {
    contract: {
        address: "0x14f69F0e10f40453857c671Df4431728aa98629f", //kovan
        abi:[
			{
				"inputs": [],
    ...
    apiKey: {
        test: "pR-0RlimZ.db2fbb41-b1bb-4fcf-9cfb-a78ff13592a1",
        prod: "pR-0RlimZ.db2fbb41-b1bb-4fcf-9cfb-a78ff13592a1"
    }
}
    ...
    const onSubmit= async event => {
        if (chestRarity > 0 && chestQuantity > 0 && contract) {
            ...
            if (metaTxEnabled) {
                ...
                let functionSignature = contract.methods.openCrate(chestRarity, chestQuantity).encodeABI();
                let message = {};
                ...
                contract.methods
                    .openCrate(chestRarity, chestQuantity)
	...
	const getOwnerTokensFromNetwork = () => {
        ...
        try {
            if (web3 && contract) {
				let userAddress = selectedAddress;
                contract.methods
                    .crazyHerosItemsOfAddress(userAddress)
                    .call()
                    .then(function (result) {
                        handleClose();
                        if (
